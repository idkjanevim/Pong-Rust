...
fn player1_move(
    ...
    mut query: Query<(&MoveSpeed, &mut Transform, With<Player1>)>,
    wins: Res<WinSize>,
    mut positions: ResMut<PositionOfPlayers>,
) {
...
fn player2_move(
    input: Res<Input<KeyCode>>,
    mut query: Query<(&MoveSpeed, &mut Transform, With<Player2>)>,
    wins: Res<WinSize>,
    mut positions: ResMut<PositionOfPlayers>,
) {
    let (speed, mut transform, _) = query.single_mut();
...
fn ball_move(
    mut query: Query<(&mut BallSpeed, &mut Transform, With<Ball>)>,
    wins: Res<WinSize>,
    mut scoreboard: ResMut<Scoreboard>,
    positions: Res<PositionOfPlayers>,
    audio: Res<Audio>,
    ...
    if ball_lcorner_x < p1_x && ball_upper_corner_y < p1_upper_y && ball_upper_corner_y > p1_lower_y
    {
        speed.0 = Vec2::new(-speed.0[0], speed.0[1]);
        audio.play(sound.0.clone());
    } else if ball_lcorner_x < p1_x
        && ball_lower_corner_y < p1_upper_y
        && ball_lower_corner_y > p1_lower_y
    {
        speed.0 = Vec2::new(-speed.0[0], speed.0[1]);
        audio.play(sound.0.clone());
    }
    ...
    if ball_rcorner_x > p2_x && ball_upper_corner_y < p2_upper_y && ball_upper_corner_y > p2_lower_y
    {
        speed.0 = Vec2::new(-speed.0[0], speed.0[1]);
        audio.play(sound.0.clone());
    } else if ball_rcorner_x > p2_x
        && ball_lower_corner_y < p2_upper_y
        && ball_lower_corner_y > p2_lower_y
    {
        speed.0 = Vec2::new(-speed.0[0], speed.0[1]);
        audio.play(sound.0.clone());
    }
